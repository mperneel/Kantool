...
class ExportDataset(tk.Toplevel):
    ...
    def __init__(self, master):
        ...
        frame_split_fields = tk.Frame(self)
    ...
    def to_psota_2019(self):
        ...
        for filename in files_list:
            if (len(filename.split(".")) == 2) and\
                ...
                (filename.split(".")[0] + ".csv" in files_list):
                #file is an image (.jpg or .png), accompanied with a .csv file
                #with annotations
            ...
            if len(files) > 0:
                ...
                for filename in files:
                    if (len(filename.split(".")) == 2) and\
                        ...
                        filename_masks = filename.split(".")[0] + "_mask.json"
                        if filename_masks in os.listdir():
                            json_file = open(filename.split(".")[0] + "_mask.json")
                        ...
                        try:
                            df_image = pd.read_csv(core + ".csv",
                                                   header=None)
                            objects_present = True
                        except pd.errors.EmptyDataError:
                            #.csv file contains no data -> no objects on image
                        ...
                        for j, keypoint in enumerate(self.skeleton.keypoints):
                            if objects_present:
                                ...
                                dict_image[keypoint] = df_image.iloc[:, 2 * j: 2 * j + 2].\
                                                        to_numpy().tolist()
                            else:
